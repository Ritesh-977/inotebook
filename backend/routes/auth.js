...
router.post('/createuser',[
    ...
    res.status(500).send("Internal Server error in Creating User")     
}
})
...
router.post('/login',[
   ...
   try {
    ...
    console.error(error.message)
    res.status(500).send("Internal Server Error in Authenticate user");
}
...
router.post('/getuser', fetchuser,  async (req, res) => {
    try {
      ...
      console.error(error.message);
      res.status(500).send("Internal Server Error in Getting Details");
    }
  })
